...
func New(cfg Config) (*Positions, error) {
...
func (p *Positions) save() {
	...
	if err := writePositionFile(p.cfg.PositionsFile, positions); err != nil {
		promtail.Errof("error writing positions file", err)
	}
}
	...
	for k := range p.positions {
		...
		if _, err := os.Stat(k); err != nil {
			if os.IsNotExist(err) {
				...
				promtail.Warnf("could not determine if log file "+
					"still exists while cleaning positions file", err)
			}
		}
	}
